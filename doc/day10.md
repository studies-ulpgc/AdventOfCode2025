...
* **Bajo Acoplamiento**: `MachineFactory` actúa como un puente entre el texto plano y los objetos de dominio, permitiendo que el resto del sistema no sepa nada sobre el formato de entrada.
...
* **Open/Closed Principle (OCP)**: El diseño permite añadir un nuevo tipo de solver (ej. `TemperatureSolver`) extendiendo la lógica sin modificar el `MachineFactory` o los modelos base.

---

## 3. Patrones y Técnicas

* **Static Factory Method**: `MachineFactory` centraliza la creación de objetos complejos de dominio, ocultando la complejidad de los `Matcher` y `Regex`.
