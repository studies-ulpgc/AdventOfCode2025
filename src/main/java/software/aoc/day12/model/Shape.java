...
public class Shape {
    ...
    private Shape rotate90() {
        return rotateGrid(grid.length, grid[0].length,
                new char[grid[0].length][grid.length]);
    }

    private Shape rotateGrid(int r, int c, char[][] next) {
        for (int i = 0; i < r; i++)
            for (int j = 0; j < c; j++)
    ...
    private Shape flip() {
        return flipGrid(grid.length, grid[0].length,
                new char[grid.length][grid[0].length]);
    }

    private Shape flipGrid(int r, int c, char[][] next) {
        for (int i = 0; i < r; i++)
            for (int j = 0; j < c; j++)
